...
#define BMP_SENSOR_CHIP_ID      0x60
/*! define sensor chip id [BMP280 = 0x58 */
#define BMP_SENSOR_CHIP_ID_C    0x60
			...
			const char *buf, size_t count)
{
	...
	int status = strict_strtoul(buf, 10, &filter);
	if (status == 0) {
		mutex_lock(&data->lock);
			...
			const char *buf, size_t count)
{
	...
	int status = strict_strtoul(buf, 10, &standbydur);
	if (status == 0) {
		mutex_lock(&data->lock);
			...
			const char *buf, size_t count)
{
	...
	int status = strict_strtoul(buf, 10, &workmode);
	if (status == 0) {
		mutex_lock(&data->lock);
			...
			const char *buf, size_t count)
{
	...
	int status = strict_strtoul(buf, 10, &enable);
	if (status == 0) {
		enable = enable ? 1 : 0;
		mutex_lock(&data->lock);
			...
			const char *buf, size_t count)
{
	...
	int status = strict_strtoul(buf, 10, &action);
	if (0 != status)
		return status;
