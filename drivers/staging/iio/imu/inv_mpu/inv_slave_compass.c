...
#define DATA_AKM_MODE_CM16			0x12 // continuous measurement in 16bit
...
/*
 *  inv_read_compass_reg() - Read akm series compass register.
 */
u8 inv_akm_read_reg(struct inv_mpu_state *st, u8 reg)
{
	u8 data;
	
	inv_i2c_read(st, reg, 1, &data);
	//inv_secondary_read(reg, 1, &data);	
	return data;
}
...
static int inv_setup_compass_akm(struct inv_mpu_state *st)
{
	...
	if (result)
		return result;*/
	result = inv_secondary_write(REG_AKM_MODE, DATA_AKM_MODE_CM16);
	if (result)
		return result;	
...
/*static int inv_mlx_read_data(struct inv_mpu_state *st, short *o)
{
	...
	z = o[2];
	// axis sensitivity conversion. Z axis has different sensitiviy from x and y 
	z *= 26;
	z /= 15;
	o[2] = z;
