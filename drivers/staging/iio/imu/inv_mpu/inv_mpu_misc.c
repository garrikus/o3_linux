...
#define DMP_IMAGE_CRC_VALUE             0xbc73ab48 //0xa139df62
	...
	u8 *data, int size)
{
	...
	pdata += MPU_DMP_LOAD_START;
	result = mem_w(memaddr, write_size, pdata);
	if (result)
		return result;
	...
	for (bank = 1; size > 0; bank++, size -= write_size, pdata += write_size) {
		...
		result = mem_w(memaddr, write_size, pdata);
		/*pr_err("write: memaddr=0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x\n", 
				memaddr, *pdata, *(pdata+1), *(pdata+2), *(pdata+3), *(pdata+4), *(pdata+5),
				*(pdata+write_size-2), *(pdata+write_size-1));*/
...
/*static int inv_verify_firmware(struct inv_mpu_state *st, u8 *data, int size)
{
	int bank, block_size;
	int result, mem_size;
	u16 memaddr;
	u8 *pdata = data;
	u8 firmware_block[MPU_MEM_BANK_SIZE];

	mem_size = size-MPU_MEM_BANK_SIZE;
	pdata += MPU_MEM_BANK_SIZE;
	pr_err("mem_size=%i\n", mem_size);
	
	for (bank = 1; mem_size > 0; bank++, mem_size -= block_size, pdata += block_size) {
		if (mem_size > MPU_MEM_BANK_SIZE)
			block_size = MPU_MEM_BANK_SIZE;
		else
			block_size = mem_size;		

		memaddr = ((bank << 8) | 0x00);
		pr_err("block_size=%i, bank=%i, memaddr=0x%02x\n", block_size, bank, memaddr);
		
		result = mpu_memory_read(st, st->i2c_addr, memaddr, block_size, firmware_block);
		
		if (result)
			return result;
			
		pr_err("read:\n");
		pr_err("read block: 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x\n", 
				firmware_block[0], firmware_block[1], firmware_block[2], firmware_block[3],
				firmware_block[block_size-4], firmware_block[block_size-3],
				firmware_block[block_size-2], firmware_block[block_size-1]); 
		pr_err("write block: 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x\n", 
				*pdata, *(pdata+1), *(pdata+2), *(pdata+3), *(pdata+4), *(pdata+5),
				*(pdata+block_size-2), *(pdata+block_size-1));
		
		if (0 != memcmp(firmware_block, pdata, block_size))
			return -EINVAL;
	}
	...
	char *buf, loff_t pos, size_t size)
{
	...
	pr_err("inv_load_firmware():\n");

	result = inv_load_firmware(st, firmware, size);
	if (result)
		goto firmware_write_fail;

	pr_err("inv_verify_firmware():\n");
