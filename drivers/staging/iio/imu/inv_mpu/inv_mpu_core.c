...
static int inv_init_config(struct iio_dev *indio_dev)
{
	...
	if (INV_ITG3500 != st->chip_type) {
		...
		result = inv_i2c_single_write(st, reg->fifo_en, d2);
		if (result)
			return result;		
		//==================================================================
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	// smike - mpu9250 - compass
	case ATTR_COMPASS_MODE_REG:		
		return sprintf(buf, "%d\n", st->slave_compass->read_reg(st, 0x64));	// INV_MPU_REG_I2C_SLV1_DO 
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	default:
		//result = kstrtoint(buf, 10, &data);
		result = strict_strtol(buf, 10, &res);
		if (result)
	...
	case ATTR_COMPASS_MATRIX:
	{
		...
		while ((str = strsep((char **)&buf, ","))) {
			...
			result = strict_strtol(str, 10, &res);
			if (result)
				goto attr_store_fail;
			if (res < -1 || res > 1) {
				result = -EINVAL;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/*{
		...
		inv_i2c_read(st, reg->fifo_en, 1, &d);
		pr_err("reg->fifo_en = 0x%02x\n", d);	
	}*/
...
// smike - mpu9250 - compass
static IIO_DEVICE_ATTR(compass_mode_reg, S_IRUGO | S_IWUSR, inv_attr_show,
	inv_attr_store, ATTR_COMPASS_MODE_REG);
...
static const struct attribute *inv_compass_attributes[] = {
	...
	&iio_dev_attr_compass_mode_reg.dev_attr.attr,
};
		...
		const struct i2c_device_id *id)
{
	...
	/*if (conf->has_compass && conf->has_pressure && (COMPASS_ID_MLX90399 == plat->sec_slave_id)) {
...
/*static int invensense_mpu_parse_dt(struct device *dev, struct mpu_platform_data *pdata)
{
	...
	dev_err(dev, "Invensense MPU parse_dt started.\n");
...
static int inv_mpu_probe(struct i2c_client *client,	const struct i2c_device_id *id)
{
	...
	indio_dev->currentmode = INDIO_DIRECT_MODE; //INDIO_BUFFER_TRIGGERED; /

	// smike
	//==================================================================
	/*gpio = of_get_named_gpio(client->dev.of_node, "inven,irq-gpio", 0);
	gpio += 22;
	
	result = gpio_request(gpio, "MPU_pin");
    if (result) {
