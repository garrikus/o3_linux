...
again:
		if (!ida_pre_get(&iio_ida, GFP_KERNEL))                 
			ret = -ENOMEM;
		
		ret = ida_get_new_above(&iio_ida, 0, &id);
        if (!ret) {
			if (id > 0x80000000) {
				ida_remove(&iio_ida, id);
                ret = -ENOSPC;
            } else {
				ret = id;
			}
...
void devm_iio_device_free(struct device *dev, struct iio_dev *iio_dev)
{
	...
	rc = devres_destroy(dev, devm_iio_device_release,
			    devm_iio_device_match, iio_dev);
	WARN_ON(rc);
}
...
void devm_iio_device_unregister(struct device *dev, struct iio_dev *indio_dev)
{
	...
	rc = devres_destroy(dev, devm_iio_device_unreg,
			    devm_iio_device_match, indio_dev);
	WARN_ON(rc);
}
