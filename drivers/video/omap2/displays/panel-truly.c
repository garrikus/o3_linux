...
static int taal_probe(struct omap_dss_device *dssdev)
{
    ...
    DSSDBG("Truly.probe(). Leave BL out.\n");

    /* So far we don't care about backlight. Considers there's no one at all. */
#if 0
...
static const unsigned char enter_sleep[2] = {0x10, 0x00};
...
static const unsigned char cmd_oscillator_setting[3] = {
      0xB0, 0x01, 0x0B,
};
static const unsigned char cmd_power_setting[20] = {
			0xB1, 0x85, 0x00, 0x34,
			0x07, 0x00, 0x0F, 0x0F,
			0x2A, 0x32, 0x3F, 0x3F,
			0x01, 0x3A, 0x01, 0xE6,
...
static const unsigned char cmd_display_setting[16] = {
			0xB2, 0x00, 0x20, 0x05,
			0x05, 0x70, 0x00, 0xFF,
			0x00, 0x00, 0x00, 0x00,
			0x03, 0x03, 0x00, 0x01
...
static const unsigned char cmd_wave_cycle_setting[6] = {0xB4, 0x00, 0x18, 0x80, 0x06, 0x02};
static const unsigned char cmd_vcom_setting[3] = {0xB6, 0x42, 0x42};

static const unsigned char cmd_gamma_setting[35] = {
			0xE0, 0x00, 0x13, 0x19,
			0x38, 0x3D, 0x3F, 0x28,
			0x46, 0x07, 0x0D, 0x0E,
			0x12, 0x15, 0x12, 0x14,
...
static const unsigned char cmd_pixel_format_setting[2] = {0x3A, 0x77};

static unsigned char cmd_column_addr[5] = {0x2A, 0x00, 0x00, 0x01, 0xDF};

static unsigned char cmd_page_addr[5] = {0x2B, 0x00, 0x00, 0x03, 0x1F};

static unsigned char img_dump[49] = { 0x2C,
...
static unsigned char img_dump3c[121] = { 0x3C,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
/*	dsi_vc_dcs_write(TCH, display_on,         				sizeof(display_on));*/
    ...
    for (i = 0; i < 100; ++i)
        dsi_vc_dcs_write(TCH, img_dump3c,         				sizeof(img_dump3c));
#endif

#if 0
    DSSDBG("jdoi-14\n");
	dsi_vc_dcs_write(TCH, write_memory_start,   			sizeof(write_memory_start));
    DSSDBG("jdoi-15\n");
#endif
...
static int _taal_enable_te(struct omap_dss_device *dssdev, bool enable)
{
    ...
    DSSDBG("EnabTE_taal: p_ext_te=%d, enable=%d", (int)(panel_data->use_ext_te), (int)enable);
	if (!panel_data->use_ext_te)
...
static int taal_enable_te(struct omap_dss_device *dssdev, bool enable)
{
    ...
    DSSDBG("EnabTE: enab=%d td->enab=%d", (int)enable, (int)(td->te_enabled));
